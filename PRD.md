# PRD — pathfind 웹 서비스 MVP (version 1.0)

> skill-to-service 킷 B 모드 환산, 11항목. 승인 완료(2026-09-10). 구현 완료(2026-09-16) 기준.

## 1) 한 줄 정의

무엇을 만들려는 사람(게임·업무 도구·문서 등 무엇이든)이, 아이디어를 명료화하는 짧은 정렬 인터뷰를 거쳐, **구현을 위한 큰 단계(기획·설계·제작·통합·테스트·출시·운영 등)와 각 단계의 할 일을 정리한 큰 그림**을 받고, 각 단계마다 **이미 있는 것(오픈소스·무료 에셋·튜토리얼·유사 사례)과 직접 해야 하는 것을 갈라 보여주는 경로**, 그리고 구현 에이전트로 이어 쓸 수 있는 패스 문서를 받는 서비스.

## 2) 타깃 사용자

- 구체적 한 사람: 퇴직을 앞두거나 막 퇴직한 50대 후반~60대 초반, 새 일을 시작해 보려는 사람. “35년 해온 목공을 온라인으로 가르쳐 보고 싶다” 같은 아이디어가 떠올랐을 때, 컴퓨터 켜고 브라우저를 열어 처음 방문해 쓴다. 스마트폰은 쓰지만 AI나 최신 도구에는 익숙하지 않고, 뭘 검색해야 하는지도 막막한 경우.
- 커버리지(무엇을 만들 때 쓸 수 있나)는 넓게 유지; MVP 데모·검증 초점은 새 일 시작 시나리오 — 작은 가게·모임·개인 기록 정리처럼 “해본 적은 없어도, 뭔가 해보려는 일”을 예시로 든다.

## 3) 문제

“무언가 새로 시작해 보고 싶다”는 생각이 들어도, 이미 비슷한 일이 어떤 식으로 이뤄지는지, 쓸 수 있는 무료 템플릿·참고 사례·쉬운 도구가 있는지 몰라서 괜히 복잡하게 시작하거나, 남들이 이미 겪은 시행착오를 다시 밟는다. 무엇보다 큰 그림과 각 단계에서 무엇을 스스로 해야 하고 무엇을 가져다 쓸 수 있는지 선택지가 보이지 않아, 생각은 있어도 첫걸음을 떼기 어렵다. 특히 컴퓨터나 최신 도구를 자주 쓰지 않던 사람이라면 “뭘 검색해야 하는지”부터 막막해서 시작 자체가 늦어진다.

## 4) 트리거

- 불러 쓰는 쪽. URL로 접속 → 첫 화면 입력 → 정렬 인터뷰(5턴 제한) → 시각화 경로 + 패스 다운로드.
- 알림·공유 링크 등으로 서비스가 먼저 가는 구조는 MVP 범위 밖. 아이디어 직후 접속 패턴이 디폴트.

## 5) 핵심 기능

**P0 (데모 필수, 최대 4개)**

1. **정렬 인터뷰** — 5턴 제한, LLM이 질문 + 추천 방향 + 예시 답변 버튼(2~3개)을 제시, 사용자는 버튼 클릭/자유 입력으로 답변. grill-me/grilling 패턴을 서비스에 흡수한 단계.
2. **큰 그림·단계 리서치** — 정렬 인터뷰 승인 뒤, 먼저 사례와 방법을 검색하고 사용자 목표와 조건에 맞는 단계를 제안한다. 검색에서 확인한 사실과 모델이 제안한 단계를 구분한다. 자료가 단계와 순서를 직접 증명해야 하는 조건은 없고, 검색 결과에서 실제로 확인한 자료(제목·링크·발췌)와 모델이 스스로 제시한 제안 이유만 문서에 남긴다. 원문 전문을 읽었다고 쓰지 않으며, 검색 결과에서 확인하지 않은 URL은 쓰지 않는다. 검색에 맞는 자료가 하나도 없으면 인터뷰 기반 초안이라는 안내를 두고 진행한다. 그다음 각 단계마다 리서치를 돌리되, 병렬 가능한 건 병렬로, 순서 의존 있는 건 순차적으로 수행한다. 각 단계 리서치 결과로 “오픈소스·프레임워크가 있어서 편해질 수 있다 / 없으면 직접 규칙·성장 시스템을 정해야 한다”, “Godot vs Unity vs 이미 나온 결과물 활용” 같은 선택지를 제시한다. 필요 시 “이 작업도 pathfind로 더 좋은 방법이 있을지 re-search 하자”는 제안을 할 수 있다.
3. **경로 시각화** — 큰 단계 카드 흐름도. 각 단계 카드에 할 일, verdict(가져다 써도 됨 / 섞어야 함 / 직접 해야 함 / 선례를 못 찾음), 찾은 자료 링크, 선택지를 담는다. 프론트에서 CSS/JS로 인터랙티브 렌더링.
4. **패스 다운로드** — 구현 에이전트로 이어 쓸 수 있는 마크다운 문서 제공. 다운로드 + 복사. 전달 대상 명칭은 일반화(“구현을 이어갈 코딩 에이전트” — 특정 제품명 미기재).

위 넷이 만든 결과(큰 그림·단계 리서치 카드·시각화·패스)는 화면에서 다시 꺼내 쓰는 층이 따로 있고, 그 목록과 세부 동작은 여섯째 절(6) 스킬 매핑에 적는다.

**P1 (이번에 안 하거나 최소화)**

- 패스 공유 링크(옵션 B)
- three.js 등 고급 시각화
- 주제별 특화 경질 프롬프트 다수

**화면 구성**

코어 화면은 세 영역이다: 왼쪽 출처/리서치 자료 목록, 중앙 정렬 인터뷰·대화, 오른쪽 단계 카드 마인드맵·패스.

## 6) 스킬 매핑

예선 스킬 원문 `pathfind-src`는 바이트 단위로 예선 제출본과 같고, 서비스화 수정과 확장은 전부 `api`와 `frontend` 코드에서 후처리로 한다.

| pathfind SKILL.md 절차(원문 단계) | pathfind-web 서비스 코드 위치 | 이식·변형·대체·미이식 | 한 줄 설명 |
|---|---|---|---|
| 판정 A/B(첫 줄 판정) — 입력 대상 여부 | `api/grill.js` | 변형(인터뷰 흡수) | 예선 원문의 첫 줄 판정 로직을 서비스에서는 grill 인터뷰로 바꿔 흡수한다. 원문의 판정 문장 형식은 프론트가 그대로 표시하지 않는다. |
| 단계 분해(단계 수 4~7, 제목 24자 이내, 실제 작업 순서) | `api/pathfind.js`(머리 주석에 원문 수록) → 프론트 렌더링 | 이식 | 큰 단계·할 일·선택지·아이콘을 bigPicture.stages로 반환하고, 카드는 프론트가 그린다. |
| 조사 절차 — 단계마다 웹 검색 1회 | `api/stage.js` + `api/_channels/`(naver·github·law·public-data·kosis) | 변형(채널·등급) | 원문은 단일 웹 검색이었으나 서비스는 규칙 기반 채널 5종과 근거 등급을 단계 텍스트 키워드로 선택한다. 채널과 등급은 stage 응답의 scope·grade로 내려간다. |
| 조사 결과 JSON 스키마(steps[], verdict, findings 등) | `api/stage.js` 출력(verdict·findings·options·todos) / `api/pathfind.js`(stages) | 이식 | 원문의 steps·verdict·findings 골격을 유지한다. verdict는 판정 4종 계약 값, findings는 kind·evidence·url을 그대로 쓴다. |
|| PATH.md — 구현 에이전트로 넘길 문서 | `api/handoff.js` → `handoffMarkdown` | 이식·확장 | 원문의 **PATH.md** 작성 의도를 마크다운 다운로드·복사로 이어간다. 서버 조립 폴백을 두고, x-handoff-source로 solar·retry·assembled 중 하나를 표기한다. |
| render.py — stdin JSON → 연구노트.md + 흐름도.html | 쓰지 않음. 큰 그림은 `api/pathfind.js`, 카드는 프론트, 패스는 `api/handoff.js` | 대체 | 웹 MVP에서는 render.py를 실행하지 않는다. 원문의 파일 생성 단계를 서비스 함수 3개로 나눠 처리한다. 매 실행에서 할 일은 `/api/stage` + 프론트 카드 렌더링 + `/api/handoff` 다운로드로 바꾼다. |
| 매 실행에서 할 일(웹검색 + render.py + share_file) | `/api/stage` + 프론트 카드 렌더링 + `/api/handoff` 다운로드 | 대체 | 원문의 한 번 실행 절차를 웹 요청 흐름으로 바꾼다. |
|| 판정 기준(무엇을 pathfind 대상으로 보는가) — 대상·대상 아님 구분과 예시로 판정 A/B를 보완 | 서비스에서는 grill 인터뷰에서 대상이 아니라고 나올 때의 안내 문장에만 원문 판정 기준 취지를 반영한다. 원문의 대상·대상 아님 예시는 프론트 안내문에만 쓰고, 서비스 판정 함수나 /api 계약에는 그대로 옮기지 않는다. | 이식 안 함 — 기준은 grill 안내로만 반영 |
| (원문에 없는 신설) 노드 설명 | `api/explain.js`(노드 설명 문단·목록·후속 질문) | 신설 — 원문 산출을 화면에서 다시 꺼내 쓰는 층 | 마인드맵 노드 하나를 누르면 중앙이 그 노드를 설명한다. 단계 노드는 판정 화면 문구(이미 있음·없음·일부만 있음·못 찾음)를 넣는다. |
| (원문에 없는 신설) 조사 후 대화 | `api/chat.js`(자료 번호 인용 답변) | 신설 — 원문 산출을 화면에서 다시 꺼내 쓰는 층 | 조사 결과를 근거 삼아 사용자가 묻고, 서버는 자료 번호를 인용한 답과 후속 질문을 돌려준다. |
| (원문에 없는 신설) 단계 항목 소주제 묶기 | `api/outline.js`(finding·task·todo를 2~4개 소주제로) | 신설 — 원문 산출을 화면에서 다시 꺼내 쓰는 층 | 단계 안의 자료·할 일·역할 나눔을 뜻이 가까운 것끼리 묶어 왼쪽 패널 문서 묶음으로 쓴다. |
| (원문에 없는 신설) 자료 카드 | `api/source-card.js`(한 자료 설명 markdown 5값) | 신설 — 원문 산출을 화면에서 다시 꺼내 쓰는 층 | 찾은 자료 하나를 한 줄 요약·무엇인가·어떻게 쓰나·피할 때·제약으로 정리해 왼쪽 패널 문서로 보여준다. |
| 원문의 단계 목록 | 왼쪽 패널의 폴더·파일 + 오른쪽 마인드맵 | 이식·확장 | 원문이 내려주던 단계 목록을 왼쪽에는 문서 묶음으로, 오른쪽에는 단계 카드 마인드맵으로 다시 꺼낸다. |

- **pathfind-src 파일 넷(SKILL.md, scripts/render.py, assets/steps-flow-template.html, assets/lucide-icons.json)** 해시 대조:

  ```
  b2e678a6d41b3538e68852296d9f3156762bf805e12fd56412c17e53988c9caf  SKILL.md
  385dc9f8bf360c038c2ee940ad1157f7cbd9455231e3407b7e3b9103a84b8f9c  scripts/render.py
  d200ff84e07f2a73782a06de90ab05da5a4508f43a4022a8b40f61ca2cfc8077  assets/steps-flow-template.html
  77611d6b324ff94af76e98eaa7048586715c165842f61375e883f9f393e4931b  assets/lucide-icons.json
  ```

  위 네 값은 현재 레포의 `pathfind-src/`에서 `sha256sum`으로 찍은 값이며, 예선 제출 zip과 파일 구성·내용이 같다는 대조 결과를 한 줄로 적어 둔 것이다. `pathfind-src/` 안에는 이 네 파일 외에 다른 파일이 없고, 서비스로 늘린 부분은 전부 `api/`와 `frontend/` 쪽 후처리다.

- 판정 4종(가져다 써도 됨·직접 해야 함·섞어야 함·선례를 못 찾음)은 계약 값이며, 저장·서버 응답에는 원문을 그대로 두고 화면에 보여 줄 때만 이미 있음·없음·일부만 있음·못 찾음으로 바꾼다.

## 7) 입력·출력

**입력**

- 첫 화면: 한 문단 텍스트 박스 + 예시 프롬프트 버튼 3~4개 (예시 후보는 가게·모임·기록 정리처럼 새 일을 시작하려는 사람의 넓은 커버리지 반영)
- 정렬 인터뷰: LLM 질문에 대해 예시 버튼 클릭 또는 자유 입력. 5턴 또는 frontier 비면 종료.
- (인터뷰·리서치 중 필요 시, pathfind 재적용 제안 수락 가능)

**출력**

- **큰 그림**: 기획·설계·제작·통합·테스트·출시·운영 등 큰 단계 카드 (각 단계별 할 일 목록, verdict, 선택지, 링크)
- **단계 리서치 결과**: 각 단계마다 가져온 자료, verdict, 선택지 (예: Godot vs Unity vs 기존 결과물 활용, 오픈소스 프레임워크 유무에 따른 난이도 판단 등)
- **시각화**: 카드 흐름도로 표시 (인터랙티브, CSS/JS)
- **패스**: 마크다운 문서 (다운로드 + 복사). 구현 에이전트로 넘길 수 있는 형태로 제공. 전달 대상 명칭은 일반화.

## 8) 필요 설정

**표 1 — 배포 연결 (Vercel 커넥터 등; 값은 취급하지 않음)**

- 정적 프론트(index.html 등) + api/*.js 서버리스 함수 구조. /api만 fetch.

**표 2 — 서비스 키 (Vercel 환경변수; 이름·용도·발급처만, 값 금지)**

| 환경변수 이름 | 용도 | 발급처 |
|---|---|---|
| `SOLAR_API_KEY` | Solar Pro 4 호출(정렬 인터뷰 5턴 + bigPicture 생성 + 단계별 리서치 + 패스 마크다운 생성). 서버 `/api`에서만 사용. 클라이언트 소스·응답 JSON·PRD·커밋 어디에도 값 미기재. | Upstage Solar API (Upstage Console / API 발급) |
| `SOLAR_API_URL` | Solar API 엔드포인트 URL. 선택. 기본값 `https://api.upstage.ai/v1/chat/completions`. | Upstage Solar API (Upstage Console / API 발급) |
| `SOLAR_MODEL` | Solar 모델 ID. 선택. 기본값 `solar-pro4`. | Upstage Solar API (Upstage Console / API 발급) |
| `SEARCH_API_KEY` | 단계별 리서치에서 웹 검색 대행(Tavily 계열 기본). `/api/stage`에서만 사용. 클라이언트 소스·응답 JSON·PRD·커밋 어디에도 값 미기재. | Tavily (Tavily API 발급) |
| `SEARCH_API_URL` | 검색 API 엔드포인트 URL. 선택. 기본값 `https://api.tavily.com/search`. | Tavily (Tavily API 발급) |
| `NAVER_CLIENT_ID` | 네이버 검색 API 클라이언트 ID. `/api/_channels/naver.js`에서 사용. 선택. | 네이버 검색 API (Naver Cloud Platform 발급) |
| `NAVER_CLIENT_SECRET` | 네이버 검색 API 클라이언트 시크릿. `/api/_channels/naver.js`에서 사용. 선택. | 네이버 검색 API (Naver Cloud Platform 발급) |
| `KOSIS_API_KEY` | KOSIS 공공데이터 API 키. `/api/_channels/kosis.js`에서 사용. 선택. | KOSIS (국가통계포털 API 발급) |
| `LAW_API_OC` | 국가법령정보센터 오픈 API 인증 파라미터. `/api/_channels/law.js`에서 사용. 선택. | 국가법령정보센터 (법제처 오픈 API 발급) |
| `GITHUB_MCP_TOKEN` | GitHub MCP 토큰. `/api/_channels/github.js`에서 사용. 선택. | GitHub (Personal Access Token 발급) |

- 실제 발급 가능 여부·구체적 값은 Vercel 환경변수 등록 시점에 확인. (값 아님; 이름·용도·발급처만 예비 기술.)
- 검색 공급자 교체 시 `api/stage.js`의 요청 형식만 바꾼다(공급자 추상화).

**표 3 — 사용자 프로필 (웹 화면 → localStorage, 재방문 시 묻지 않음, 초기화 버튼)**

- 최초 입력 텍스트, 예시 버튼 선택, 정렬 인터뷰 Q&A 누적(턴별), 재방문 시 이어서 사용 가능하게 저장. 초기화 버튼 제공.

## 9) 데이터 규칙

- 출처·기준 시각 표기 (찾은 자료의 URL·검색어·발췌 근거 포함 — pathfind 규약 계승).
- 없는 값은 ‘확인 불가’로 표시, 추정 금지.
- 키·프록시 주소는 서버(/api)에만. 클라이언트는 같은 도메인 /api만 fetch.
- 최소 권한.

## 10) 성공 기준 (데모 시나리오로 검증 가능한 조건)

**P0 기준**

- 계정·저장값 없는 시크릿 창 첫 방문에서, 첫 화면이 스스로 보이고 예시 버튼+텍스트 입력 가능.
- 예시 버튼 또는 직접 입력으로 정렬 인터뷰 진입 → 5턴 이내(또는 frontier 비면) 종료 → 큰 그림·단계 리서치 카드 + 패스 다운로드가 나옴.
- pathfind 스킬의 데이터 경로(큰 단계 + 각 단계 verdict·findings·선택지)가 서비스 /api에서 실제로 생성·사용됨.
- 클라이언트 소스에 키·프록시 주소 0.

**검증 방법 (실제 확인)**

- 프로덕션 도메인에서 index 200, 제목 문자열 원문 그대로, /api 200 직접 확인.
- 시크릿 창 테스트: 저장값 없는 첫 방문에서 안내가 스스로 나오고 최소 기능(입력→인터뷰→큰 그림/단계 리서치→결과) 동작.

## 11) 제외 범위

- 알림·공유 링크 등 서비스가 먼저 가는 기능 (MVP 밖)
- 패스 공유 링크의 외부 구현 에이전트 실제 도달 보장 (MVP 미보장)
- three.js 등 고급 시각화
- 특정 코딩 에이전트 제품명(Claude Code, Codex 등) 언급 — MVP에선 일반화로 대체
- Solar API 키 값, 그 밖의 비밀 값 (어디에도 미기재)

---

## 대응표 (걸음 ①~⑤ → PRD 항목)

| 걸음 | 사용자의 답(요약) | PRD 항목 |
|------|-------------------|----------|
| ① WHO | 20대 후반, 게임 좋아하는 비코딩 직장인, “풋볼매니저 야구 버전 만들고 싶다” 아이디어 직후 | 2) 타깃 사용자 |
| ② WHEN | 아이디어 직후, 컴퓨터 켜고 브라우저 열었을 때 첫 10분. URL 접속형, 알림·공유 링크은 MVP 밖 | 4) 트리거 |
| ③ INPUT/OUTPUT | 텍스트 박스+예시 버튼 3~4개 → 5턴 Solar 정렬 인터뷰(질문+추천+예시 버튼) → 카드 흐름도 시각화 + 패스 다운로드 | 7) 입력·출력, 5) 핵심 기능 |
| ④ SETUP | 비밀: LLM 호출 키(서버 /api 전용, 값 미기재). 프로필: localStorage(Q&A 누적 등), 초기화 버튼. 웹 검색 대행 서버. render.py → 웹 출력 로직 변환. grill-me는 서비스 흡수 단계 | 8) 필요 설정, 6) 스킬 매핑 |
| ⑤ SOLVE | 없던 때: 막연한 검색·포기·바퀴 재발명·시행착오 반복·품질 미달·시간/토큰 낭비. 있으면: 시행착오 회피, 오픈소스/에셋 활용, 아이디어 구체화, 단계별 할 일·순서 제시, 패스로 구현 에이전트 연결(일반화) | 1) 한 줄 정의, 3) 문제, 5) 핵심 기능, 11) 제외 범위 |

---

## 해결된 항목 (version 1.0)

- 패스 공유 링크(옵션 B): PATH.md 내려받기 + 미리보기로 대체. 공유 링크형 MVP 범위에서 닫힘.
- LLM 호출용 키 (`SOLAR_API_KEY`): 버셀에 환경변수 등록 완료. 값은 PRD·클라이언트·커밋 어디에도 미기재.
- 웹 검색 키 (`SEARCH_API_KEY`): Tavily 기준으로 버셀 환경변수 등록 완료. `/api/stage`에서만 사용.
- 큰 그림·단계 리서치 오케스트레이션 방식: `/api/pathfind.js`(큰 그림) + `/api/stage.js`(단계별 리서치, 채널 5종 프리서치 병렬 + 분석 호출, 호출 상한 8회) + `/api/handoff.js`(마크다운 생성)로 구성. 프리서치는 규칙 기반 채널 선택 후 병렬로 수행한다.
- render.py 웹 변환 상세: MVP에서는 render.py를 사용하지 않는다. bigPicture JSON은 `api/pathfind.js`가 Solar로 직접 생성, 단계 카드는 프론트, **PATH.md**는 `api/handoff.js`가 생성한다.

---

## 사용 자산과 출처

화면에 쓴 아이콘과 글꼴.

- 아이콘: lucide (https://lucide.dev)
- 글꼴: Noto Sans KR (https://fonts.google.com/noto/specimen/Noto+Sans+KR)

조사에 쓰는 바깥 공개 API 다섯. 주소는 아래와 같다.

- 네이버 검색 (https://developers.naver.com)
- GitHub (https://docs.github.com/en/rest)
- 공공데이터포털 (https://www.data.go.kr)
- KOSIS (https://kosis.kr)
- 국가법령정보센터 (https://open.law.go.kr)

우리가 만든 것이라 출처를 밖에 둘 필요가 없는 것들.

- 화면 부품: ui.askewly.com 레지스트리 설치본 — 우리 것.
- 화면 갈무리와 도식: 직접 만든 것.

---

## 버전·업그레이드 방침

이 PRD는 version 1.0. 구현·데모 과정에서 더 쉽고 나은 길이 발견되면 이 PRD를 수정한다(규정상 “안 읽고 만들면 엉뚱한 것 나온다” 주의는 유지 — 수정 시 A-1 걸음과 정합 재확인).

---

*승인: 2026-09-10 (skill-to-service B 모드 → C-계획 승인)*
*파일: PRD.md · 이 레포 루트에 위치*
