...
"했다"는 말은 완료가 아니다. 일반 구현 카드는 지정 파일의 diff와 변경 줄 수 및 커밋 파일과 해시를 보고하고 끝낸다. 타입 검사·빌드·보안 스캔은 별도로 지정한 검증 카드에서만 수행한다. 내가 "확인해서 결과 보여 줘"라고 하면 이 뜻이다.

## 커밋 게이트

커밋 전 `git status`로 dirty·untracked를 확인한다. `.env*`·키·토큰·프록시 주소·개인정보가 포함되면 커밋하지 않는다. 단, 이 커밋 게이트는 일반 구현 카드에는 적용하지 않으며, 타입 검사와 보안 스캔은 별도로 지정한 검증 카드에서만 수행한다. 인증과 비밀정보 보호, 지정 파일만 커밋하는 규칙은 그대로 지킨다.
...
## 카드 작업의 실행 범위
  ...
  카드 작업에서는 이 절을 기존의 추가 탐색과 자동 검증 지시보다 우선한다. 인증과 비밀정보 보호 및 수정 금지 범위는 그대로 지킨다.

  Hermes는 지정된 구현과 커밋만 맡는다. 사전 조사와 완료 검증은 카드 관리자가 맡는다.

  ### 읽기와 구현

  - 현재 카드가 지정한 파일과 함수, 직접 연결된 사용처만 읽는다.

  - 레포 전체 탐색, 다른 카드 조사, 별도 설계와 리팩터링은 하지 않는다.

  - 지정한 이름이 없거나 실제 코드와 지시가 다르면 파일과 위치를 보고하고 멈춘다. 추측해서 대체 구현을 만들지 않는다.

  - 카드 범위 밖 오류는 수정하지 않는다.

  ### 실행하지 않는 작업

  - 타입 검사, 빌드, 린트, 테스트, 보안 스캔을 실행하지 않는다.

  - 개발 서버와 브라우저를 띄우거나 외부 API를 호출하지 않는다.

  - 검사 환경, 임시 시험 파일, 가짜 서버를 만들거나 패키지를 설치하지 않는다.

  - 별도로 명시된 검증 작업을 받은 경우에만 지정된 검사를 수행한다. 실패했다고 다른 검사를 추가하거나 반복하지 않는다.

  ### 변경 확인과 커밋

  - 작업 전후 지정 파일의 diff와 변경 줄 수를 확인한다.

  - 기존 편집은 카드가 인수를 명시한 범위만 함께 커밋한다.

  - 다른 사람의 편집이 섞여 구분할 수 없으면 포함하거나 되돌리지 말고 보고한다.

  - 지정 파일만 커밋한다. 전체 파일을 한꺼번에 스테이징하지 않는다.

  - 변경이 없으면 빈 커밋을 만들지 않는다.

  ### 보고와 종료

  - 카드가 지정한 첫 줄과 커밋 메시지 형식을 따른다.

  - 변경 파일, 파일별 추가 및 삭제 줄 수, 커밋 해시를 보고한다.

  - 실행 검증을 하지 않았다면 그대로 적는다. 구현 완료를 기능 검증 통과로 표현하지 않는다.

  - 구현과 커밋 보고가 끝나면 종료한다. 다음 카드 착수, 추가 조사, 개선 작업은 하지 않는다.