...
대여섯 개를 한 커밋 단위로 쪼개고, **A(빌드 기반·진입)는 오케스트레이터가 직접 처리**, **B∥C(컴포넌트·모듈)는 delegate_task로 병렬 스폰**한다. 의존성: A → (B∥C) → D(상태·훅 교체) → E(문서 정리).

위임 실행 규약:
- delegate_task `tasks`는 **JSON 배열**이어야 한다. 문자열이면 파싱 오류로 계속 실패한다.
- 각 태스크 인자는 `goal`(완료 조건 포함) + `context`(사실·소유 파일·금지 사항) + `output_schema`를 명시한다. 서브에이전트는 자기 콘텍스트에 아무것도 모르므로 필요한 사실을 전부 문안에 넣는다.
- 위임 시작 전 오케스트레이터가 기반을 먼저 확보한다: 파일 읽기, 레지스트리 fetch, 계약 문서 확인, 의존성 최신화.
- 위임 산출물은 즉시 커밋하지 않는다. 오케스트레이터가 **diff + 실행 출력 + 계약 일치**로 직접 검증한 뒤에 커밋한다.
- 서브에이전트 커밋은 오케스트레이터만 수행한다. 위임된 태스크는 파일 생성·수정·터미널 실행만 하고, 커밋은 오케스트레이터 몫이다.

판정:
- 서브에이전트 산출물 판정은 `npm run build` 성공 여부와 `npx @askewly/design verify <대상 디렉터리>` 출력으로 한다.
- 최종 게이트는 사람 확인(라이브 페이지). 기계 검사 통과만으로 완료로 치지 않는다.
- 진입 프로토콜: 코드 자산 JSON fetch 실패 시 조용히 폴백하지 않고 보고한다.

위임 결과 수신 후 오케스트레이터가 하는 일:
1. 생성 파일 존재·크기 확인.
2. 핵심 파일 직접 읽기(셸 content 원문 일치, 순수 함수 여부, 계약 정합성).
3. 빌드 성공 확인.
4. verify 결과 확인(위반 있으면 resolve 방향 정리).
5. 문제 없으면 커밋, 문제 있으면 재작업 또는 방향 조정.
