...
완료 조건: fetch 호출 4개의 위치(파일:행)를 표로 적고, 사람이 확인할 조작 순서 3줄. 로컬 http.server 에는 API 가 없으니 완주 확인은 오케스트레이터 커밋 뒤 https://pathfind.askewly.com/app.html 에서 사람이 한다. git 은 쓰지 않는다.
