...
function TopbarButton({ children, icon, primary, ...rest }: { children: ReactNode; icon?: ReactNode; primary?: boolean } & React.ComponentProps<"button">) {
  return (
    <button
      type="button"
      className={cn(
...
export function CharacterAvatar({ size = 32 }: { size?: number }) {
  return (
...
// ── 상단 바 (M121) ────────────────────────────────────────────────────────────
// 셸은 `topbar` 를 ReactNode 슬롯으로 넘긴다. 그런데 정작 이 바가 데모 안에만 있어서, 소비자는 매번 새로
// 그리고 **로그인 상태와 제자리 편집 제목을 잃었다**(mabc-2026 실측 — 평범한 h1 로 다시 그렸다).
// 그래서 내보낸다. 다만 원본 데모의 액션 5개(복사·분석·공유·설정·앱 격자)는 **기본값으로 서지 않는다** —
// 기능 없는 버튼을 세우는 것은 「상태 없는 인터랙션 요소」로 시그니처 hard-fail 이다. 액션은 소비자가 준 것만 선다.

export type NotebookTopbarAction = {
  id: string
  label: string
  icon?: ReactNode
  onClick?: () => void
  /** 검은 채움 CTA — 한 바에 하나만 둔다(초점 1개) */
  primary?: boolean
  disabled?: boolean
  title?: string
}

export type NotebookUser = { name: string }

export type NotebookTopbarProps = {
  title: string
  /** 주면 제자리 편집이 켜진다. 안 주면 읽기 전용 제목 */
  onTitleChange?: (title: string) => void
  titleFallback?: string
  /** 제목 오른쪽 액션. **기본값 없음** — 소비자가 준 것만 그린다 */
  actions?: NotebookTopbarAction[]
  /** 액션과 로그인 사이의 자유 슬롯(남은 횟수 배지·상태 칩 등) */
  statusSlot?: ReactNode
  /**
   * 로그인 영역. `onSignIn` 을 주지 않으면 **로그인 자리를 아예 그리지 않는다** —
   * 인증이 없는 제품에 죽은 「로그인」 버튼을 세우지 않기 위해서다.
   */
  user?: NotebookUser | null
  onSignIn?: () => void
  onSignOut?: () => void
  /** 아바타 자리 — 기본은 브랜드 캐릭터. 실제 프로필 이미지가 있으면 넣는다 */
  avatar?: ReactNode
  labels?: { title?: string; signIn?: string; signOut?: string }
  className?: string
}

/**
 * 노트북 상단 바 — 제자리 편집 제목(왼쪽) + 액션·상태·로그인(오른쪽), 셸의 64h 슬롯에 그대로 들어간다.
 * 관측 원천은 M117 §1-2(원본의 구글 아바타 자리 — 사진 대신 브랜드 캐릭터, 사용자 주문 2026-09-13).
 */
export function NotebookTopbar({ title, onTitleChange, titleFallback = "제목 없는 노트북", actions = [], statusSlot, user, onSignIn, onSignOut, avatar, labels, className }: NotebookTopbarProps) {
  const L = { title: "노트북 제목", signIn: "로그인", signOut: "로그아웃", ...labels }
  return (
    <div className={cn("flex w-full items-center justify-between gap-4 px-4", className)}>
      {onTitleChange ? (
        <EditableText
          as="h1"
          value={title}
          ...
          aria-label={L.title}
          data-shell-title
          className="min-w-0 max-w-[40%] truncate font-normal focus:overflow-visible focus:whitespace-normal"
          style={{ fontSize: 22, lineHeight: "36px" }}
        />
      ) : (
        <h1 data-shell-title className="min-w-0 max-w-[40%] truncate font-normal" style={{ fontSize: 22, lineHeight: "36px" }}>
          {title}
        </h1>
      )}
      <div className="flex shrink-0 items-center gap-3">
        {actions.map((a) => (
          <TopbarButton key={a.id} icon={a.icon} primary={a.primary} onClick={a.onClick} disabled={a.disabled} title={a.title} data-shell-action={a.id}>
            {a.label}
          </TopbarButton>
        ))}
        {statusSlot}
        {onSignIn &&
          (user ? (
            <button
              type="button"
              data-shell-avatar
              aria-label={`${user.name} — ${L.signOut}`}
              title={`${user.name} · ${L.signOut}`}
              onClick={onSignOut}
              className="inline-flex items-center justify-center rounded-full text-foreground outline-none ring-2 ring-primary/40 ring-offset-2 ring-offset-background motion-safe:transition-[box-shadow] hover:ring-primary focus-visible:ring-ring"
            >
              {avatar ?? <CharacterAvatar size={AVATAR_PX} />}
            </button>
          ) : (
            <button
              type="button"
              data-shell-signin
              onClick={onSignIn}
              className="inline-flex h-8 items-center rounded-full border border-border px-4 text-sm font-medium text-foreground outline-none ring-ring ring-offset-2 ring-offset-background motion-safe:transition-colors hover:bg-foreground/8 focus-visible:ring-2"
            >
              {L.signIn}
            </button>
          ))}
...
/**
 * 상단 바 데모 — 내보낸 `NotebookTopbar` 로 원본 관측 형태를 재현한다. 액션 5개는 **데모가 주입한 것**이고
 * 부품 기본값이 아니다(M121 — 기본값으로 죽은 버튼을 세우지 않는다).
 */
function DemoTopbar({ title, onTitleChange, user, onSignIn, onSignOut }: { title: string; onTitleChange: (t: string) => void; user: DemoUser | null; onSignIn: () => void; onSignOut: () => void }) {
  return (
    <NotebookTopbar
      ...
      onSignOut={onSignOut}
      actions={[
        { id: "new", label: "노트북 만들기", icon: <Plus aria-hidden />, primary: true },
        { id: "copy", label: "복사", icon: <Copy aria-hidden /> },
        { id: "analytics", label: "분석", icon: <ChartNoAxesCombined aria-hidden /> },
        { id: "share", label: "공유", icon: <Share2 aria-hidden /> },
        { id: "settings", label: "설정", icon: <Settings aria-hidden /> },
      ]}
      statusSlot={
 ...
 */
export function MindmapPanel({ root, layout = "roadmap", mapTitle, onMapTitleChange, sourcesLabel, onShowSources, onShare, onMore, expandedIds, onExpandedChange, selectedId, onSelectedChange, onNodeSelect, collapsed, onCollapsedChange, headerSlot, legend, labels: labelsProp, className }: MindmapPanelProps) {
